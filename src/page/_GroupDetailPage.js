...
import {AppCore,share,loadIfEmpty,goTo} from '../util/core';
...
class GroupDetailPage extends Component{
	constructor(props) {
		...
		this.url = '/api/App/group_detail/'+props.p.id;
	}
	afterLoad() {
	...
	holdSeat(){
		let id = this.state.data['团期详情'][0]['id'];
		goTo('占位订单',{data:{id:id},action:'占位订单'});
	}

	// 实报页面
	realSignUp(){
		let group = this.state.data['团期详情'][0];
				    ...
				    <div className="posi-rela">
						...
						<div className="pro-header-info">
							...
							<div className="pro-price">
								<div className="pro-price-zk_price">￥{(this.state.data['团期详情'][0] && this.state.data['团期详情'][0].zk_price) * 1} <span style={{fontSize: '.373333rem', fontWeight: 'normal'}}>起</span></div>
								<div className="pro-price-dep_city">{this.state.data['团期详情'].dep_city_name}出发</div>
							</div>
							<div className="pro-sale">
								<div className="pro-sale-price">{ (this.state.data['团期详情'][0] && this.state.data['团期详情'][0].peer_price) }</div>
								<div className="pro-sale-supplier">供应商: {this.state.data['团期详情'].pd_provider}</div>
							</div>
						</div>
						

						{/* 团期日历 */}
						<Calendar groups={this.state.data['团期详情']}></Calendar>
...
export default connect(s=>({s:s}))(GroupDetailPage)